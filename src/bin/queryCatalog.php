...
if (!isset($config['slack']['webhooks']) || $config['slack']['webhooks'][0] == 'YOUR_SLACK_WEB_HOOK_GOES_HERE'){
    echo "\nYou have not yet specified the slack url to send runtime errors to.\n";
    exit;
}
...
if (is_readable($strCacheFile)) {
    try {
        ...
        echo "\nFailed to read or write the local storage file to cache directory.\nPlease make sure this script can write to this directory.\n";
        exit;
    }
}
else {
    $virgin = true;
    echo "\nNOTICE: Looks like this is the first time executing.  Local cache will been populated.  Going forward, new extensions will notify.\n\n";
}


// Adobe IMS and AdobeIO Authentication
$adobeIO = new AdobeIO($config['adobe']['io']['ims_end_point']);
$access_token = $adobeIO->getAccessToken($config['adobe']['io']['client_id'], $config['adobe']['io']['client_secret']);

if(!is_string($access_token) || strlen($access_token) <= 0) {
    $errMsg = "\nFailed to obtain a valid IMS Token. Please check that the credentials are valid and that IMS is functional.\n";
    echo $errMsg;
    ...
    if ($verbose) {
        echo "Failed to get access_token. Exiting...\n";
    }
    exit;
}
